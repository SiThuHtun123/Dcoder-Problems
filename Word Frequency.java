...
public class ProblemSolving {
	public static void main(String[] args) {
		...
		int f = 0;
		for (Integer l : indexArray) {
			for (int i = 0; i <= Integer.parseInt(copyArray[l]) - 1; i++) {
				f = Arrays.asList(stringArray).indexOf(myList.get(l));
				stringArray = ArrayUtils.remove(stringArray, f);
			}

		}
		for (int g = 0; g <= stringArray.length - 1; g++) {
			result += stringArray[g] + " ";
		}
